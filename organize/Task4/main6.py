...
# Expand the 'Strategy' and 'Topic2' columns into separate rows
...
def plot_data(df, title, line_filename, stacked_area_filename):
    ...
    plt.savefig(f"{line_filename}7.svg", format='svg')
    ...
    plt.savefig(f"{stacked_area_filename}7.svg", format='svg')
    plt.close(fig)  # Close the figure to free up memory

# Example usage
plot_data(strategy_counts_per_year, 'Strategy Counts', 'strategy_counts_line', 'strategy_counts_stacked_area')
plot_data(topic_counts_per_year, 'Privacy Topic Counts', 'topic_counts_line', 'topic_counts_stacked_area')
