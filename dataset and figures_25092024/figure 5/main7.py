...
def plot_gender_distribution(df, gender, output_filename):
    ...
    # Define specific colors for each Topic2
    paper_type_colors = {
        'Security': 'red',
        'Privacy': 'blue',
    }
    ...
    # Plot each Topic2 separately
    for paper_type, color in paper_type_colors.items():
        subset = df[df['Topic2'] == paper_type]
        marker = 'o'
    ...
    #ax.set_title(f'{gender} Participant Distribution by S&P concerns')
    ...
    ax.legend(handles=paper_type_handles, title='S&P concern', loc='lower right')
    ...
    for fmt in ['pdf', 'png', 'svg']:
        plt.savefig(f"{output_filename}9.{fmt}", bbox_inches='tight', dpi=300)  # Higher DPI for better clarity
